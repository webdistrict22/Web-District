export const translations = {
  en: {
    brand: {
      tagline: "Strategic Web Agency",
    },
    welcome: {
      aria: "Welcome to Web District",
      title: "Welcome to Web District",
      subline: "Let's build something your brand can grow with.",
    },
    nav: {
      links: {
        home: "Home",
        services: "Services",
        work: "Work",
        process: "Process",
        start: "Start",
        contact: "Contact",
      },
      login: "Login",
      account: "Account",
      clientAccount: "Client account",
      adminDashboard: "Admin dashboard",
      startProject: "Start Your Project",
      openMenu: "Open mobile menu",
      closeOverlay: "Close mobile menu overlay",
      closeMenu: "Close mobile menu",
      ticker: "WEBSITES YOUR BRAND GROWS WITH",
      languageToggle: "عربي",
    },
    footer: {
      text: "We don't just build websites. We craft digital work that feels like art.",
      help: "Help",
      contact: "Contact",
      terms: "Terms & Conditions",
      privacy: "Privacy Policy",
      whatsapp: "WhatsApp",
    },
    common: {
      buttons: {
        startProject: "Start Your Project",
        viewWork: "View Our Work",
        viewWorkShort: "View Work",
        viewServices: "View services",
        answerQuestions: "Answer Your Questions",
        viewQuestions: "View Questions & Answers",
        contactWebDistrict: "Contact Web District",
        websiteCare: "Website Care",
        backHome: "Back home",
        backToWork: "Back to work",
        open: "Open",
        cancel: "Cancel",
        startRequest: "Start a request",
        newRequest: "New request",
        submitRequest: "Submit a request",
        bookCall: "Book a call",
      },
      loading: {
        default: "Loading...",
        page: "Loading",
      },
      labels: {
        clientPortal: "Client portal",
        adminNote: "Admin note",
        notSet: "Not set",
        notAdded: "Not added",
        notAddedYet: "Not added yet.",
        phone: "Phone",
        email: "Email",
        business: "Business",
        businessName: "Business name",
        name: "Name",
        deadline: "Deadline",
        budget: "Budget",
        preferredContact: "Preferred contact",
        brandIdentity: "Brand identity",
        contentReady: "Content ready",
        callAppointment: "Call appointment",
        callDate: "Call date",
        time: "Time",
        yourNotes: "Your notes",
        clientNote: "Client note",
        paymentNotes: "Payment notes",
        totalPrice: "Total price",
        deposit: "Deposit",
        remaining: "Remaining",
        pagesIncluded: "Pages included",
        featuresIncluded: "Features included",
        accountType: "Account type",
      },
      empty: {
        title: "Nothing here yet",
        description: "Once there is data, it will appear here.",
      },
      stages: {
        current: "Current stage",
        completed: "Completed stage",
        upcoming: "Upcoming stage",
      },
      statuses: {
        New: "New",
        Read: "Read",
        Replied: "Replied",
        Archived: "Archived",
        Reviewed: "Reviewed",
        Accepted: "Accepted",
        Rejected: "Rejected",
        Draft: "Draft",
        Sent: "Sent",
        "In Progress": "In Progress",
        Completed: "Completed",
        Cancelled: "Cancelled",
        Pending: "Pending",
        Approved: "Approved",
        Rescheduled: "Rescheduled",
        Done: "Done",
        "Contract Sent": "Contract Sent",
      },
      websiteTypes: {
        "Online Store": "Online Store",
        "Business Website": "Business Website",
        "Landing Page": "Landing Page",
        "Custom Website": "Custom Website",
      },
      contactMethods: {
        WhatsApp: "WhatsApp",
        "Phone Call": "Phone Call",
        Email: "Email",
        Instagram: "Instagram",
      },
      yesNo: {
        Yes: "Yes",
        No: "No",
        "Not sure": "Not sure",
        Partially: "Partially",
      },
      accountTypes: {
        Admin: "Admin",
        Client: "Client",
      },
    },
    home: {
      metaTitle: "Strategic Web Agency",
      metaDescription:
        "Web District builds clean, professional websites for brands, businesses, campaigns, and custom digital needs.",
      hero: {
        headline: "Your brand, brought online with care.",
        subline:
          "Elegant websites for brands ready to look more polished, trusted, and complete online.",
        primaryCTA: "Start Your Project",
        secondaryCTA: "View Our Work",
      },
      services: {
        eyebrow: "What we build",
        title: "Four clear website directions.",
        description:
          "Pick the direction that matches the goal. We keep the structure clean from there.",
        titles: [
          "Online Stores",
          "Business Websites",
          "Landing Pages",
          "Custom Websites",
        ],
      },
      work: {
        eyebrow: "Some of our work",
        title: "Selected builds, shown simply.",
        description:
          "Real examples with public websites, dashboards, and launch-ready flows.",
        openCaseStudy: "Open case study",
        comingSoon: "Case study coming soon",
        ariaOpen: "Open {name} case study",
      },
      finalCta: {
        eyebrow: "Start",
        title: "Ready to build something serious?",
        description:
          "Tell us what you need. We'll help shape the right website direction.",
        tryIt: "Try it yourself",
      },
      reviews: {
        eyebrow: "Reviews",
        title: "Our clients words.",
        description: "A word from clients who worked with Web District.",
        loading: "Loading reviews...",
        emptyTitle: "No approved reviews yet.",
        emptyDescription:
          "Approved reviews will appear here after they are added or submitted by clients.",
        rating: "Rating",
        client: "Client",
      },
    },
    services: {
      metaTitle: "Services",
      metaDescription:
        "Explore Web District website services including online stores, business websites, landing pages, and custom websites.",
      hero: {
        eyebrow: "Services",
        title: "Websites built around the business goal.",
        description:
          "You choose the website direction. We shape the structure, visuals, and launch path.",
      },
      loading: "Loading website options...",
      cards: [
        {
          title: "Online Stores",
          label: "E-commerce",
          description:
            "Websites built to make browsing and buying products feel clear.",
          longDescription:
            "For brands that need a polished product flow and a serious buying experience.",
          includes: [
            "Home, shop, and product details",
            "Cart and checkout flow",
            "Admin dashboard",
          ],
          bestFor: ["Product brands", "Local stores", "Online shops"],
        },
        {
          title: "Business Websites",
          label: "Companies",
          description:
            "Professional websites that explain the business fast and earn trust.",
          longDescription:
            "For companies, service providers, and teams that need trust and leads.",
          includes: ["Clear pages", "Lead forms", "Trust sections"],
          bestFor: ["Companies", "Service providers", "Consultants"],
        },
        {
          title: "Landing Pages",
          label: "Campaigns",
          description: "Focused pages for launches, offers, ads, and lead capture.",
          longDescription:
            "For campaigns that need one clear message and one clear action.",
          includes: ["Sharp message", "Strong CTA", "Great visuals"],
          bestFor: ["Ads", "Launches", "Lead generation"],
        },
        {
          title: "Custom Websites",
          label: "Systems",
          description:
            "Tailored web builds for workflows that need more than pages.",
          longDescription:
            "For projects that need dashboards, booking, portals, or custom logic.",
          includes: ["Custom builds", "Dashboards", "Booking flows"],
          bestFor: ["Client portals", "Booking systems", "Custom workflows"],
        },
      ],
      care: {
        eyebrow: "Website Care",
        title: "Keep your website polished after launch.",
        description:
          "Small updates, fixes, and improvements handled monthly so your website keeps feeling fresh, reliable, and easy to manage.",
        ask: "Ask About Website Care",
        included: "What's included",
        monthly: "Monthly care plans",
        monthlyTitle: "Support that matches your update rhythm.",
        customQuote: "Custom quote",
        monthlySupport: "Monthly support based on your website needs.",
        askWhatsapp: "Ask on WhatsApp",
        notIncludedLabel: "Not included:",
        notIncluded:
          "Major redesigns, large new features, full system rebuilds, or emergency 24/7 support are quoted separately unless agreed.",
        whatsappMessage: "Hi Web District, I want to ask about Website Care.",
        items: [
          {
            title: "Small content edits",
            description:
              "Update short text, offers, contact details, sections, or repeated content.",
          },
          {
            title: "Image or text updates",
            description:
              "Swap images, adjust copy, and keep important pages feeling current.",
          },
          {
            title: "Website health checks",
            description: "Review key pages, forms, links, and responsive behavior.",
          },
          {
            title: "Small layout improvements",
            description: "Refine compact areas without rebuilding the whole website.",
          },
          {
            title: "Support and guidance",
            description:
              "Ask questions and get clear direction when your website needs attention.",
          },
        ],
        plans: [
          {
            title: "Essential Care",
            bestFor: "Best for small text/image updates.",
            features: [
              "Monthly small edits",
              "Basic website checks",
              "Image or copy swaps",
              "Email or WhatsApp support",
            ],
          },
          {
            title: "Growth Care",
            bestFor: "Best for active brands and stores.",
            features: [
              "Frequent content updates",
              "Offer or product support",
              "Small layout refinements",
              "Monthly improvement notes",
            ],
          },
          {
            title: "Priority Care",
            bestFor:
              "Best for businesses that need faster support and ongoing improvements.",
            features: [
              "Faster support window",
              "Ongoing refinement tasks",
              "Regular health checks",
              "Roadmap guidance",
            ],
          },
        ],
      },
      bottomCta: {
        eyebrow: "Not sure what you need?",
        title: "Start with the goal.",
        description:
          "Tell us what the website should do. We'll shape the right structure.",
      },
    },
    work: {
      hero: {
        eyebrow: "Some of our work",
        title: "Selected websites with a serious direction.",
        description:
          "A look at selected builds across stores, business sites, and custom directions.",
      },
      loading: "Loading selected work...",
      openCaseStudy: "Open case study",
      comingSoon: "Case study coming soon",
      ariaOpen: "Open {name} case study",
      caseStudy: {
        loading: "Loading case study...",
        eyebrow: "Case study",
        notFoundTitle: "Case study not found",
        notFoundDescription:
          "This project may be coming soon, hidden, or the link may be incorrect.",
        showcase: "Showcase",
        showcaseTitle: "case-study views.",
        screen: "Screen",
        publicWebsite: "Public Website",
        publicFeatures: "Public website features",
        adminDashboard: "Admin / Dashboard",
        adminFeatures: "Admin/dashboard features",
        fallbackDetail: "Details will be added soon.",
      },
      projects: {
        zohour: {
          type: "Online Store",
          businessType: "Floral cap store",
          status: "Featured",
          description:
            "A calm e-commerce store for floral-inspired caps with a soft brand system.",
          overview:
            "A polished store experience built around product discovery, checkout, and order tracking.",
          publicFeatures: [
            "Homepage with floral vibes",
            "Shop and product details",
            "Cart and checkout flow",
            "Customer orders area",
          ],
          adminFeatures: [
            "Product manager",
            "Order and shipping controls",
            "Offers and discount codes",
            "Waitlist and analytics views",
          ],
          pages: [
            "Home",
            "Shop",
            "Product Details",
            "Cart",
            "Checkout",
            "My Orders",
            "Admin Orders",
            "Admin Products",
          ],
          techHighlights: [
            "MERN storefront",
            "Protected customer/admin routes",
            "Discount and order calculations",
            "Responsive product grid",
          ],
          tags: ["E-commerce", "Online Store", "Brand"],
          showcaseTitles: [
            "Premium Storefront",
            "Product Discovery",
            "Checkout Flow",
            "Admin Operations",
          ],
        },
        "s8-factory": {
          type: "Business Website",
          businessType: "Clothing factory",
          status: "Featured",
          description:
            "A bold business website for a clothing factory with a direct path to start.",
          overview:
            "A serious factory presence with clear production paths and operational admin tools.",
          publicFeatures: [
            "Home page with bold presence",
            "Brands and partners showcase",
            "Material lines presentation",
            "Production request and call flows",
          ],
          adminFeatures: [
            "Production request manager",
            "Call appointments and slot settings",
            "Contracts and proposals",
            "Reviews and analytics",
          ],
          pages: [
            "Home",
            "Brands",
            "Lines",
            "Appointment",
            "About",
            "Contact",
            "Account",
            "Admin",
          ],
          techHighlights: [
            "MERN request workflow",
            "Protected account/admin routes",
            "Appointment settings",
            "Contract tracking",
          ],
          tags: ["Business Website", "Factory", "Portal"],
          showcaseTitles: [
            "Business Homepage",
            "Production Lines",
            "Request And Call Flow",
            "Admin Workspace",
          ],
        },
        atheer: {
          type: "Online Store",
          businessType: "Perfume store",
          status: "Featured",
          description:
            "A dark luxury perfume store with products, checkout, orders, and admin tools.",
          overview:
            "A fragrance storefront with a strong visual direction and a practical commerce backend.",
          publicFeatures: [
            "Dark luxury homepage",
            "Shop and product details",
            "Cart and checkout",
            "Customer order tracking",
          ],
          adminFeatures: [
            "Orders dashboard",
            "Products and product uploads",
            "Offers and discount codes",
            "Analytics and customer management",
          ],
          pages: [
            "Home",
            "Shop",
            "Product Details",
            "Cart",
            "Checkout",
            "My Orders",
            "Contact",
            "Admin",
          ],
          techHighlights: [
            "Perfume product metadata",
            "Payment proof fields",
            "Discount code flow",
            "Cloudinary-ready uploads",
          ],
          tags: ["E-commerce", "Perfume", "Admin"],
          showcaseTitles: [
            "Luxury Storefront",
            "Fragrance Browsing",
            "Order Experience",
            "Store Management",
          ],
        },
        akm: {
          type: "Online Store",
          businessType: "Clothing store",
          status: "Featured",
          description:
            "A clean fashion store with product variants, offers, checkout, and tracking.",
          overview:
            "A commerce build focused on product variants, customer orders, and serious admin operations.",
          publicFeatures: [
            "Homepage and Arabic translation",
            "Shop and product details",
            "Color and size variants",
            "Cart and checkout",
          ],
          adminFeatures: [
            "Products manager",
            "Orders and payment statuses",
            "Offers, bundles, coupons and reviews",
            "Email campaigns and site settings",
          ],
          pages: [
            "Home",
            "Shop",
            "Product Details",
            "Cart",
            "Checkout",
            "My Orders",
            "Admin Products",
            "Admin Orders",
          ],
          techHighlights: [
            "React Query data flows",
            "Color/size stock model",
            "Meta/TikTok/Snap tracking hooks",
            "Cloudinary upload service",
          ],
          tags: ["E-commerce", "Fashion", "Tracking"],
          showcaseTitles: [
            "Fashion Storefront",
            "Product Variants",
            "Shopping Flow",
            "Admin Dashboard",
          ],
        },
      },
    },
    process: {
      hero: {
        eyebrow: "Process",
        title: "A clear path from request to launch.",
        description:
          "Simple steps, clear process, and a website direction everyone understands.",
      },
      questionsHeader: {
        eyebrow: "Questions",
        title: "Before we start.",
        description: "Quick answers about how the project moves.",
      },
      cta: {
        eyebrow: "Start with clarity",
        title: "Tell us the goal.",
        description:
          "We'll help shape the right website direction from there.",
      },
      steps: [
        {
          number: "01",
          title: "Request",
          subtitle: "Start with the basics.",
          description:
            "Send the business goal, website type, and anything you already know.",
          points: ["Website type", "Business goal", "Timeline"],
        },
        {
          number: "02",
          title: "Direction",
          subtitle: "Shape the plan.",
          description:
            "We define the pages, features, content needs, and visual direction.",
          points: ["Page structure", "Main CTA", "Feature scope"],
        },
        {
          number: "03",
          title: "Build",
          subtitle: "Design and develop.",
          description:
            "The website is built with a clean interface, responsive layouts, and working flows.",
          points: ["Responsive frontend", "Forms or flows", "Admin tools if needed"],
        },
        {
          number: "04",
          title: "Launch",
          subtitle: "Test and go live.",
          description:
            "We check mobile, forms, links, and launch details before the website goes public.",
          points: ["Mobile testing", "Form checks", "Launch support"],
        },
      ],
      questions: [
        {
          question: "What happens after I send a request?",
          answer:
            "We review the goal, website type, and details you send. Then we suggest the best direction and next step.",
        },
        {
          question: "Do I need everything ready before starting?",
          answer:
            "No. If you have the main idea, we can help shape the pages, structure, and content direction from there.",
        },
        {
          question: "Can I book a call first?",
          answer:
            "Yes. If you want to talk through the project before submitting details, you can start with a call.",
        },
        {
          question: "How do revisions work?",
          answer:
            "We share the work in clear stages, collect your notes, and adjust the website before launch.",
        },
        {
          question: "What happens after launch?",
          answer:
            "We make sure the website is live, working properly, and ready for people to visit.",
        },
      ],
    },
    start: {
      hero: {
        eyebrow: "Start",
        title: "Start Your Project.",
        description:
          "Send the details now, or book a call if you want to shape the direction first.",
      },
      loggedIn: {
        title: "You're starting as {name}",
        description:
          "Your requests and appointments will appear in your dashboard.",
        button: "View my requests",
      },
      options: {
        eyebrow: "Choose your way",
        title: "Start with the path that fits.",
        description:
          "Send the request now, or book a call if the direction needs shaping first.",
        footer:
          "Logged-in clients can track requests and appointments from their dashboard.",
        items: [
          {
            id: "request",
            title: "Send a project request",
            description: "Best if you already know the website you need.",
          },
          {
            id: "call",
            title: "Book a call",
            description: "Best if you want to discuss the direction first.",
          },
        ],
      },
      requestForm: {
        eyebrow: "Website request",
        title: "Tell us what you need.",
        description:
          "Share the business, goal, and website direction. We'll review it and contact you with the next step.",
        name: "Name *",
        namePlaceholder: "Your name",
        businessName: "Business name",
        businessNamePlaceholder: "Business / brand name",
        phone: "Phone / WhatsApp *",
        phonePlaceholder: "011...",
        email: "Email *",
        emailPlaceholder: "you@example.com",
        websiteType: "Website type *",
        preferredContact: "Preferred contact method",
        identity: "Do you already have a logo / identity?",
        content: "Do you have content/products/images?",
        budget: "Budget range",
        optional: "Optional",
        deadline: "Deadline",
        details: "Project details *",
        detailsPlaceholder:
          "Tell us about your business, website goal, key pages, and important features.",
        submit: "Submit website request",
        submitting: "Submitting...",
        validation:
          "Please fill name, phone, email, and project details.",
        successLoggedIn:
          "Website request submitted. You can track it in your dashboard.",
        success: "Website request submitted successfully.",
        error: "Failed to submit website request.",
      },
      callForm: {
        eyebrow: "Book a call",
        title: "Choose a time to talk.",
        description:
          "Pick an available call slot and tell us what you want to discuss.",
        bookingAs:
          "You are booking as {name}. This appointment will appear in your client dashboard.",
        availableSlots: "Available slots",
        topic: "What do you want to discuss? *",
        topicPlaceholder:
          "Example: I want an online store for my brand",
        notes: "Optional notes",
        notesPlaceholder: "Any extra details before the call?",
        submit: "Book call appointment",
        submitting: "Booking...",
        myAppointments: "My appointments",
        validationSlot: "Please choose a call slot.",
        validationDetails:
          "Please fill name, phone, email, and discussion topic.",
        loadError: "Failed to load available call slots.",
        successLoggedIn:
          "Call appointment booked. You can track it in your dashboard.",
        success: "Call appointment booked successfully.",
        error: "Failed to book appointment.",
      },
      slots: {
        loading: "Loading available call slots...",
        emptyTitle: "No call slots available yet.",
        emptyDescription:
          "You can still submit a website request or contact Web District directly through WhatsApp.",
        chooseDay: "Choose a day",
        nextDays: "Next 5 available days",
        available: "Available",
      },
    },
    contact: {
      hero: {
        eyebrow: "Contact",
        title: "Tell us what you need.",
        description:
          "Message us directly, or use the Start page if you already know the website direction.",
      },
      whatsappMessage:
        "Hi Web District, I want to ask about building a website.",
      cards: [
        {
          title: "WhatsApp",
          description: "Message us directly.",
        },
        {
          title: "Phone",
          description: "Call for a quick conversation.",
        },
        {
          title: "Email",
          description: "Send project details.",
        },
        {
          title: "Instagram",
          description: "Follow or send a DM.",
        },
      ],
    },
    auth: {
      login: {
        eyebrow: "Login",
        title: "Access your Web District account.",
        description:
          "Login to track requests, appointments, contracts, and project updates.",
        email: "Email",
        password: "Password",
        passwordPlaceholder: "Enter your password",
        forgotPassword: "Forgot password?",
        submit: "Login",
        submitting: "Logging in...",
        noAccount: "Don't have an account?",
        createOne: "Create one",
        validation: "Please enter email and password.",
        error: "Failed to login.",
      },
      signup: {
        eyebrow: "Signup",
        title: "Create your client account.",
        description:
          "Use your account later to track requests, appointments, contracts, and project updates.",
        password: "Password *",
        passwordPlaceholder: "Minimum 6 characters",
        submit: "Create account",
        submitting: "Creating account...",
        haveAccount: "Already have an account?",
        login: "Login",
        validation: "Please enter name, email, and password.",
        passwordLength: "Password must be at least 6 characters.",
        error: "Failed to create account.",
      },
      forgot: {
        eyebrow: "Password reset",
        title: "Reset your password.",
        description:
          "Enter your account email and we will send a secure reset link if the account exists.",
        submit: "Send reset link",
        submitting: "Sending...",
        remember: "Remember your password?",
        success: "Reset instructions sent.",
        validation: "Please enter your email.",
        error: "Failed to send reset instructions.",
      },
      reset: {
        eyebrow: "New password",
        title: "Create a new password.",
        description:
          "Choose a secure password to regain access to your Web District account.",
        newPassword: "New password",
        confirmPassword: "Confirm password",
        confirmPlaceholder: "Repeat your new password",
        submit: "Reset password",
        submitting: "Resetting...",
        goBack: "Go back to",
        validation: "Please enter and confirm your new password.",
        noMatch: "Passwords do not match.",
        passwordLength: "Password must be at least 6 characters.",
        success: "Password reset successfully.",
        error:
          "Failed to reset password. The link may be invalid or expired.",
      },
    },
    client: {
      layout: {
        portal: "Client portal",
        welcome: "Welcome, {name}",
        goHome: "Go home",
        logout: "Logout",
        links: {
          overview: "Overview",
          requests: "Requests",
          appointments: "Appointments",
          contracts: "Contracts",
          projectStatus: "Project Status",
          reviews: "Reviews",
          profile: "Profile",
        },
      },
      dashboard: {
        eyebrow: "Overview",
        title: "Your Web District workspace.",
        description:
          "Track your website requests, call appointments, reviews, and future contracts from one clean client portal.",
        submitNew: "Submit a new request",
        viewRequests: "View requests",
        businessFallback: "Not added",
        actions: [
          {
            title: "Website requests",
            description: "See the requests you submitted while logged in.",
          },
          {
            title: "Call appointments",
            description: "Track booked calls and appointment status.",
          },
          {
            title: "Reviews",
            description:
              "Submit a review for Web District after working together.",
          },
          {
            title: "Profile",
            description: "View your account and business information.",
          },
        ],
        nextEyebrow: "What happens next?",
        nextTitle: "Start with a request or a call.",
        nextDescription:
          "If you already know what website you need, submit a website request. If you are still unsure, book a call and Web District will help shape the right direction before moving to a proposal or contract.",
        steps: [
          "Submit a request",
          "Web District reviews it",
          "We clarify scope and next step",
          "Proposal or contract is prepared",
        ],
      },
      requests: {
        title: "Your website requests",
        description:
          "Track the requests you submitted to Web District and follow their current status.",
        loading: "Loading your website requests...",
        loadError: "Failed to load your requests.",
        emptyTitle: "No website requests yet",
        emptyDescription:
          "When you submit a website request, it will appear here with its current status.",
      },
      appointments: {
        title: "Your call appointments",
        description:
          "Track your booked calls with Web District and follow their current status.",
        bookAnother: "Book another call",
        loading: "Loading your call appointments...",
        loadError: "Failed to load your appointments.",
        emptyTitle: "No call appointments yet",
        emptyDescription:
          "When you book a call appointment, it will appear here with its current status and call details.",
      },
      contracts: {
        title: "Contracts and proposals",
        description:
          "View proposals, project scope, payment details, timeline, and contract status prepared by Web District. You can accept a proposal or send a note from here.",
        loading: "Loading your contracts...",
        loadError: "Failed to load your contracts.",
        emptyTitle: "No contracts yet",
        emptyClient:
          "Contracts and proposals prepared by Web District will appear here.",
        emptyAdmin:
          "Contracts you create from requests or appointments will appear here.",
        acceptTitle: "Accept proposal?",
        acceptMessage:
          "This will mark the Web District proposal as accepted and notify the team.",
        acceptConfirm: "Accept",
        acceptedSuccess: "Contract accepted successfully.",
        acceptError: "Failed to accept contract.",
        noteSaved: "Your note was saved successfully.",
        noteError: "Failed to save note.",
        acceptProposal: "Accept proposal",
        accepting: "Accepting...",
        sendNote: "Send / edit note",
        noteLabel: "Your note to Web District",
        notePlaceholder:
          "Write any question, comment, or clarification about this proposal...",
        saveNote: "Save note",
        saving: "Saving...",
      },
      projectStatus: {
        title: "Project status",
        description:
          "Track the progress of proposals and active projects from contract status.",
        viewContracts: "View contracts",
        loading: "Loading project status...",
        loadError: "Failed to load project status.",
        emptyTitle: "No active project status yet",
        emptyDescription:
          "Once Web District prepares a proposal or contract for you, project status will appear here.",
      },
      reviews: {
        title: "Submit a review",
        description:
          "Share your experience with Web District. Your review will appear publicly only after admin approval.",
        note:
          "Reviews are available for client accounts with at least one contract. Every review is checked before it appears publicly.",
        checking: "Checking review access...",
        loadError: "Failed to check contract access.",
        lockedTitle: "Review access is locked for now.",
        lockedDescription:
          "Your account needs at least one Web District contract before you can submit a review.",
        businessPlaceholder: "Your business / brand",
        role: "Role",
        rolePlaceholder: "Founder / Owner / Manager",
        rating: "Rating",
        stars: "{count} stars",
        message: "Review message *",
        messagePlaceholder:
          "Example: Web District helped us launch a cleaner and more professional website.",
        submit: "Submit review",
        submitting: "Submitting...",
        validation: "Please write your review message.",
        success: "Review submitted and waiting for approval.",
        error: "Failed to submit review.",
      },
      profile: {
        title: "Profile information",
        description:
          "This is the account information connected to your Web District workspace. Requests, calls, reviews, and future contracts are linked to this account.",
        startNew: "Start a new request",
        clientFallback: "Client",
        noBusiness: "No business name added",
        noteEyebrow: "Account note",
        noteTitle: "Keep your requests linked.",
        noteDescription:
          "When you submit a website request or book a call while logged in, it appears in your client dashboard automatically. If you submit while logged out, it will still reach Web District, but it will not appear inside this account.",
        quickCards: [
          {
            title: "Requests",
            description: "View website requests linked to your account.",
          },
          {
            title: "Calls",
            description: "Track booked appointments and call status.",
          },
          {
            title: "Reviews",
            description: "Submit a review after working with Web District.",
          },
        ],
      },
    },
    success: {
      metaTitle: "Success",
      metaDescription:
        "Your Web District request was submitted successfully.",
      next: "What happens next",
      startAnother: "Start another",
      call: {
        eyebrow: "Call booked",
        title: "Your call appointment is booked.",
        description:
          "We received your call details. We will review the direction and contact you if anything needs confirmation.",
        steps: [
          "We review your call topic.",
          "We prepare the right questions before the call.",
          "We use the call to shape the website direction.",
        ],
        accountLabel: "View appointment",
      },
      request: {
        eyebrow: "Request sent",
        title: "Your website request has been sent.",
        description:
          "We received your project details. We will review the business goal, website type, and direction before replying.",
        steps: [
          "We review your website request.",
          "We shape the best direction for the project.",
          "We contact you with the next step.",
        ],
        accountLabel: "View request",
      },
    },
    legal: {
      terms: {
        metaTitle: "Terms & Conditions",
        metaDescription:
          "Read Web District's clear service terms for website requests, proposals, payments, timelines, revisions, launch, and monthly support.",
        eyebrow: "Terms",
        title: "Terms & Conditions",
        description:
          "Simple service terms for working with Web District, from first request to launch and support.",
        ctaEyebrow: "Contact",
        ctaTitle: "Questions about these terms?",
        ctaDescription:
          "Start a project when you are ready, contact Web District with any questions, or review monthly Website Care inside our services for support after launch.",
        sections: [
          {
            title: "Services",
            body: [
              "Web District provides website design, development, online store, landing page, and custom web services depending on each project scope.",
              "The exact service details are agreed before work begins.",
            ],
          },
          {
            title: "Project requests and proposals",
            body: [
              "Submitting a request, form, or message does not mean a project is automatically accepted.",
              "After reviewing the details, Web District may prepare a proposal, quotation, or contract based on the project scope.",
            ],
          },
          {
            title: "Payments and deposits",
            body: [
              "Payment terms, deposits, milestones, and delivery timing are agreed per project.",
              "Work may depend on receiving the agreed deposit or payment before a stage begins.",
            ],
          },
          {
            title: "Timelines and delays",
            body: [
              "Project timelines are planned around the agreed scope, content, feedback, and technical needs.",
              "Delays in client feedback, content, images, approvals, or third-party access can affect delivery timelines.",
            ],
          },
          {
            title: "Client responsibilities",
            body: [
              "Clients are responsible for providing correct business information, website content, images, brand assets, account access, and approvals.",
              "If Web District helps shape content direction, the client is still responsible for confirming accuracy before launch.",
            ],
          },
          {
            title: "Revisions and small changes",
            body: [
              "Reasonable revisions are handled according to the project agreement and stage of work.",
              "Small changes are different from major new features, page rebuilds, or a change in direction unless agreed separately.",
            ],
          },
          {
            title: "Website content and assets",
            body: [
              "Clients should only provide content, images, logos, fonts, or assets they own or have permission to use.",
              "Web District may showcase completed work in its portfolio unless the client requests otherwise.",
            ],
          },
          {
            title: "Third-party services",
            body: [
              "Projects may use third-party services such as hosting, domains, email, analytics, payment platforms, plugins, or APIs.",
              "Those services may have their own pricing, terms, limits, and availability.",
            ],
          },
          {
            title: "Website launch",
            body: [
              "Before launch, Web District checks the agreed pages, responsive layout, core links, and main forms or flows.",
              "Final launch depends on access, approvals, domain or hosting readiness, and any third-party services involved.",
            ],
          },
          {
            title: "Monthly support / website care",
            body: [
              "Monthly support or website care is separate unless it is included in the project agreement.",
              "Care plans may cover small edits, updates, checks, and guidance. Major new features are not included as small monthly edits unless agreed.",
            ],
          },
        ],
      },
      privacy: {
        metaTitle: "Privacy Policy",
        metaDescription:
          "Learn how Web District collects and uses information submitted through forms, client accounts, messages, and website requests.",
        eyebrow: "Privacy",
        title: "Privacy Policy",
        description:
          "A clear look at the information we collect and how it helps us respond, plan, and support your website project.",
        ctaEyebrow: "Contact",
        ctaTitle: "Need to ask about your data?",
        ctaDescription:
          "Start a project when you are ready, or contact Web District if you have a privacy question before moving forward.",
        sections: [
          {
            title: "Information we collect",
            body: [
              "We may collect details you submit, such as name, email, phone number, WhatsApp number, business name, website needs, appointment details, and project information.",
              "If you create a client account, we may also store account details needed to manage your dashboard and requests.",
            ],
          },
          {
            title: "How we use information",
            body: [
              "We use information to respond to messages, review project requests, manage appointments, prepare proposals or contracts, deliver services, and communicate about your project.",
              "We may also use general website activity to improve the website experience.",
            ],
          },
          {
            title: "Forms and website requests",
            body: [
              "When you send a website request or contact form, the information helps us understand your goals and suggest the right next step.",
              "Please avoid sending sensitive personal information unless it is necessary for your request.",
            ],
          },
          {
            title: "Client accounts",
            body: [
              "Client accounts may show requests, appointments, contracts, project status, reviews, and profile information connected to your work with Web District.",
              "Account access should be kept private by the user.",
            ],
          },
          {
            title: "Cookies / analytics note",
            body: [
              "The website may use basic cookies, analytics, or similar tools to understand performance and visitor activity.",
              "These tools help us improve the site and do not change the price or availability of our services.",
            ],
          },
          {
            title: "Email and communication",
            body: [
              "If you contact us by email, WhatsApp, Instagram, phone, or website form, we may keep the conversation so we can respond and manage the project properly.",
              "You can ask us to update or remove unnecessary contact details.",
            ],
          },
          {
            title: "Data storage",
            body: [
              "Information may be stored in our website system, email inbox, project records, hosting environment, or approved business tools.",
              "We aim to keep information only as long as needed for communication, service delivery, records, or reasonable business purposes.",
            ],
          },
          {
            title: "Sharing information",
            body: [
              "We do not sell personal information.",
              "We may use third-party services such as hosting, email, analytics, payment/contact platforms, or technical tools when needed to operate the website and deliver services.",
            ],
          },
          {
            title: "User rights",
            body: [
              "You can contact us to ask about the information we hold, request corrections, or ask for information to be removed where reasonably possible.",
              "Some records may need to be kept for project, contract, security, or business reasons.",
            ],
          },
        ],
      },
    },
    notFound: {
      metaTitle: "Page Not Found",
      metaDescription:
        "The page you are looking for does not exist on Web District.",
      title: "This page went off-grid.",
      description:
        "The page you are looking for does not exist, was moved, or the link is incorrect.",
    },
  },
  ar: {
    brand: {
      tagline: "وكالة ويب استراتيجية",
    },
    welcome: {
      aria: "مرحبًا بك في Web District",
      title: "مرحبًا بك في Web District",
      subline: "لنبنِ شيئًا تنمو معه علامتك التجارية.",
    },
    nav: {
      links: {
        home: "الرئيسية",
        services: "الخدمات",
        work: "أعمالنا",
        process: "الخطوات",
        start: "ابدأ",
        contact: "تواصل معنا",
      },
      login: "تسجيل الدخول",
      account: "الحساب",
      clientAccount: "حساب العميل",
      adminDashboard: "Admin dashboard",
      startProject: "ابدأ مشروعك",
      openMenu: "افتح القائمة",
      closeOverlay: "إغلاق خلفية القائمة",
      closeMenu: "إغلاق القائمة",
      ticker: "مواقع تنمو مع علامتك التجارية",
      languageToggle: "EN",
    },
    footer: {
      text: "نصمم مواقع لا تبدو جميلة فقط، بل تشعر وكأنها عمل رقمي متقن.",
      help: "روابط مهمة",
      contact: "تواصل",
      terms: "الشروط والأحكام",
      privacy: "سياسة الخصوصية",
      whatsapp: "WhatsApp",
    },
    common: {
      buttons: {
        startProject: "ابدأ مشروعك",
        viewWork: "شاهد أعمالنا",
        viewWorkShort: "شاهد الأعمال",
        viewServices: "شاهد الخدمات",
        answerQuestions: "إجابات لأسئلتك",
        viewQuestions: "الأسئلة والإجابات",
        contactWebDistrict: "تواصل مع Web District",
        websiteCare: "العناية بالموقع",
        backHome: "العودة للرئيسية",
        backToWork: "العودة للأعمال",
        open: "فتح",
        cancel: "إلغاء",
        startRequest: "ابدأ طلبًا",
        newRequest: "طلب جديد",
        submitRequest: "إرسال طلب",
        bookCall: "احجز مكالمة",
      },
      loading: {
        default: "جار التحميل...",
        page: "جار التحميل",
      },
      labels: {
        clientPortal: "بوابة العميل",
        adminNote: "ملاحظة من الإدارة",
        notSet: "غير محدد",
        notAdded: "غير مضاف",
        notAddedYet: "لم تتم الإضافة بعد.",
        phone: "الهاتف",
        email: "البريد الإلكتروني",
        business: "النشاط",
        businessName: "اسم النشاط",
        name: "الاسم",
        deadline: "الموعد النهائي",
        budget: "الميزانية",
        preferredContact: "طريقة التواصل المفضلة",
        brandIdentity: "الهوية البصرية",
        contentReady: "جاهزية المحتوى",
        callAppointment: "موعد مكالمة",
        callDate: "تاريخ المكالمة",
        time: "الوقت",
        yourNotes: "ملاحظاتك",
        clientNote: "ملاحظة العميل",
        paymentNotes: "ملاحظات الدفع",
        totalPrice: "إجمالي السعر",
        deposit: "الدفعة المقدمة",
        remaining: "المتبقي",
        pagesIncluded: "الصفحات الموجودة",
        featuresIncluded: "المميزات الموجودة",
        accountType: "نوع الحساب",
      },
      empty: {
        title: "لا يوجد شيء هنا بعد",
        description: "عند توفر بيانات، ستظهر هنا.",
      },
      stages: {
        current: "المرحلة الحالية",
        completed: "مرحلة مكتملة",
        upcoming: "مرحلة قادمة",
      },
      statuses: {
        New: "جديد",
        Read: "تمت القراءة",
        Replied: "تم الرد",
        Archived: "مؤرشف",
        Reviewed: "تمت المراجعة",
        Accepted: "مقبول",
        Rejected: "مرفوض",
        Draft: "مسودة",
        Sent: "مرسل",
        "In Progress": "قيد التنفيذ",
        Completed: "مكتمل",
        Cancelled: "ملغي",
        Pending: "قيد الانتظار",
        Approved: "معتمد",
        Rescheduled: "تمت إعادة الجدولة",
        Done: "مكتمل",
        "Contract Sent": "تم إرسال العقد",
      },
      websiteTypes: {
        "Online Store": "متجر إلكتروني",
        "Business Website": "موقع تعريفي للأعمال",
        "Landing Page": "صفحة هبوط",
        "Custom Website": "موقع مخصص",
      },
      contactMethods: {
        WhatsApp: "WhatsApp",
        "Phone Call": "مكالمة هاتفية",
        Email: "البريد الإلكتروني",
        Instagram: "Instagram",
      },
      yesNo: {
        Yes: "نعم",
        No: "لا",
        "Not sure": "لست متأكدًا",
        Partially: "جزئيًا",
      },
      accountTypes: {
        Admin: "Admin",
        Client: "عميل",
      },
    },
    home: {
      metaTitle: "وكالة ويب استراتيجية",
      metaDescription:
        "Web District تصمم مواقع احترافية للعلامات التجارية والأعمال والحملات والمشاريع الرقمية المخصصة.",
      hero: {
        headline: "نحضر علامتك التجارية إلى الإنترنت بعناية.",
        subline:
          "مواقع أنيقة للعلامات التجارية التي تريد حضورًا أونلاين أكثر احترافًا وثقة واكتمالًا.",
        primaryCTA: "ابدأ مشروعك",
        secondaryCTA: "شاهد أعمالنا",
      },
      services: {
        eyebrow: "ما الذي نصممه",
        title: "أربع اتجاهات واضحة للمواقع.",
        description:
          "اختر الاتجاه الأقرب لهدفك، ونحن نبني الهيكل والتجربة حوله بعناية.",
        titles: [
          "متاجر إلكترونية",
          "مواقع تعريفية للأعمال",
          "صفحات هبوط",
          "مواقع مخصصة",
        ],
      },
      work: {
        eyebrow: "من أعمالنا",
        title: "مشاريع مختارة، معروضة ببساطة.",
        description:
          "أمثلة حقيقية لمواقع عامة ولوحات تحكم وتجارب جاهزة للإطلاق.",
        openCaseStudy: "عرض المشروع",
        comingSoon: "دراسة المشروع قريبًا",
        ariaOpen: "عرض مشروع {name}",
      },
      finalCta: {
        eyebrow: "ابدأ",
        title: "جاهز لبناء شيء جاد؟",
        description:
          "أخبرنا بما تحتاجه، وسنساعدك في اختيار الاتجاه المناسب للموقع.",
        tryIt: "جربه بنفسك",
      },
      reviews: {
        eyebrow: "آراء العملاء",
        title: "كلمات من عملائنا.",
        description: "آراء من عملاء عملوا مع Web District.",
        loading: "جار تحميل الآراء...",
        emptyTitle: "لا توجد آراء معتمدة بعد.",
        emptyDescription:
          "ستظهر الآراء المعتمدة هنا بعد إضافتها أو إرسالها من العملاء.",
        rating: "التقييم",
        client: "عميل",
      },
    },
    services: {
      metaTitle: "الخدمات",
      metaDescription:
        "تعرّف على خدمات Web District في تصميم المتاجر الإلكترونية والمواقع التعريفية وصفحات الهبوط والمواقع المخصصة.",
      hero: {
        eyebrow: "الخدمات",
        title: "مواقع مبنية حول هدف العمل.",
        description:
          "أنت تختار اتجاه الموقع، ونحن نشكل الهيكل والتصميم ومسار الإطلاق.",
      },
      loading: "جار تحميل خيارات المواقع...",
      cards: [
        {
          title: "متاجر إلكترونية",
          label: "E-commerce",
          description:
            "مواقع تجعل تصفح المنتجات والشراء تجربة واضحة وسهلة.",
          longDescription:
            "للعلامات التجارية التي تحتاج تجربة منتجات أنيقة وشراء جاد.",
          includes: [
            "الرئيسية والمتجر وتفاصيل المنتج",
            "السلة ومسار الدفع",
            "لوحة تحكم للإدارة",
          ],
          bestFor: ["علامات المنتجات", "المتاجر المحلية", "المتاجر الأونلاين"],
        },
        {
          title: "مواقع تعريفية للأعمال",
          label: "شركات",
          description:
            "مواقع احترافية تشرح النشاط بسرعة وتبني الثقة.",
          longDescription:
            "للشركات ومقدمي الخدمات والفرق التي تحتاج حضورًا واضحًا وموثوقًا.",
          includes: ["صفحات واضحة", "نماذج تواصل", "أقسام لبناء الثقة"],
          bestFor: ["الشركات", "مقدمو الخدمات", "الاستشاريون"],
        },
        {
          title: "صفحات هبوط",
          label: "حملات",
          description:
            "صفحات مركزة للإطلاقات والعروض والإعلانات وجمع العملاء المحتملين.",
          longDescription:
            "للحملات التي تحتاج رسالة واحدة واضحة وإجراء واحد واضح.",
          includes: ["رسالة قوية", "دعوة واضحة لاتخاذ إجراء", "Visuals أنيقة"],
          bestFor: ["الإعلانات", "الإطلاقات", "جمع العملاء المحتملين"],
        },
        {
          title: "مواقع مخصصة",
          label: "أنظمة",
          description:
            "حلول ويب مصممة للعمليات التي تحتاج أكثر من مجرد صفحات.",
          longDescription:
            "للمشاريع التي تحتاج لوحات تحكم أو حجوزات أو بوابات أو منطق مخصص.",
          includes: ["بناء مخصص", "لوحات تحكم", "مسارات حجز"],
          bestFor: ["بوابات العملاء", "أنظمة الحجز", "تدفقات عمل مخصصة"],
        },
      ],
      care: {
        eyebrow: "العناية بالموقع",
        title: "حافظ على موقعك أنيقًا ومحدّثًا بعد الإطلاق.",
        description:
          "تحديثات بسيطة، إصلاحات، وتحسينات شهرية تساعد موقعك أن يظل حديثًا وموثوقًا وسهل الإدارة.",
        ask: "اسأل عن العناية بالموقع",
        included: "ما الذي يشمله",
        monthly: "خطط العناية الشهرية",
        monthlyTitle: "دعم يناسب إيقاع تحديثاتك.",
        customQuote: "تسعير مخصص",
        monthlySupport: "دعم شهري حسب احتياج موقعك.",
        askWhatsapp: "اسأل عبر WhatsApp",
        notIncludedLabel: "غير مشمول:",
        notIncluded:
          "إعادة التصميم الكبيرة، المميزات الجديدة الواسعة، إعادة بناء النظام بالكامل، أو الدعم الطارئ طوال اليوم يتم تسعيرها بشكل منفصل ما لم يتم الاتفاق عليها.",
        whatsappMessage: "مرحبًا Web District، أريد الاستفسار عن العناية بالموقع.",
        items: [
          {
            title: "تعديلات محتوى بسيطة",
            description:
              "تحديث نصوص قصيرة أو عروض أو بيانات التواصل أو أقسام متكررة.",
          },
          {
            title: "تحديثات صور أو نصوص",
            description:
              "تبديل الصور، تحسين النصوص، والحفاظ على الصفحات المهمة بشكل حديث.",
          },
          {
            title: "فحص صحة الموقع",
            description:
              "مراجعة الصفحات الأساسية والنماذج والروابط والتجاوب على الشاشات.",
          },
          {
            title: "تحسينات تخطيط بسيطة",
            description:
              "تحسين مساحات صغيرة في الموقع دون إعادة بنائه بالكامل.",
          },
          {
            title: "دعم وتوجيه",
            description:
              "اطرح أسئلتك واحصل على توجيه واضح عندما يحتاج موقعك إلى عناية.",
          },
        ],
        plans: [
          {
            title: "العناية الأساسية",
            bestFor: "مناسبة لتحديثات النصوص والصور البسيطة.",
            features: [
              "تعديلات بسيطة شهرية",
              "فحوصات أساسية للموقع",
              "تبديل صور أو نصوص",
              "دعم عبر البريد أو WhatsApp",
            ],
          },
          {
            title: "العناية للنمو",
            bestFor: "مناسبة للعلامات والمتاجر النشطة.",
            features: [
              "تحديثات محتوى متكررة",
              "دعم للعروض أو المنتجات",
              "تحسينات تخطيط بسيطة",
              "ملاحظات تحسين شهرية",
            ],
          },
          {
            title: "العناية المتقدمة",
            bestFor: "مناسبة للأعمال التي تحتاج دعمًا أسرع وتحسينات مستمرة.",
            features: [
              "نافذة دعم أسرع",
              "مهام تحسين مستمرة",
              "فحوصات دورية للموقع",
              "توجيه لخطة التطوير",
            ],
          },
        ],
      },
      bottomCta: {
        eyebrow: "لست متأكدًا مما تحتاجه؟",
        title: "ابدأ من الهدف.",
        description:
          "أخبرنا بما يجب أن يحققه الموقع، وسنشكّل الهيكل المناسب.",
      },
    },
    work: {
      hero: {
        eyebrow: "من أعمالنا",
        title: "مواقع مختارة باتجاه واضح وجاد.",
        description:
          "نظرة على مشاريع مختارة بين المتاجر والمواقع التعريفية والحلول المخصصة.",
      },
      loading: "جار تحميل الأعمال المختارة...",
      openCaseStudy: "عرض المشروع",
      comingSoon: "دراسة المشروع قريبًا",
      ariaOpen: "عرض مشروع {name}",
      caseStudy: {
        loading: "جار تحميل دراسة المشروع...",
        eyebrow: "دراسة مشروع",
        notFoundTitle: "لم يتم العثور على دراسة المشروع",
        notFoundDescription:
          "قد يكون هذا المشروع قادمًا قريبًا أو مخفيًا أو أن الرابط غير صحيح.",
        showcase: "العرض",
        showcaseTitle: "لقطات من المشروع.",
        screen: "لقطة",
        publicWebsite: "الموقع العام",
        publicFeatures: "مميزات الموقع للزوار",
        adminDashboard: "لوحة التحكم",
        adminFeatures: "مميزات لوحة التحكم",
        fallbackDetail: "ستتم إضافة التفاصيل قريبًا.",
      },
      projects: {
        zohour: {
          type: "متجر إلكتروني",
          businessType: "متجر قبعات بطابع زهري",
          status: "مميز",
          description:
            "متجر إلكتروني هادئ لقبعات مستوحاة من الزهور مع هوية ناعمة.",
          overview:
            "تجربة متجر مصقولة مبنية حول اكتشاف المنتجات والدفع وتتبع الطلبات.",
          publicFeatures: [
            "صفحة رئيسية بطابع زهري",
            "المتجر وتفاصيل المنتج",
            "السلة ومسار الدفع",
            "منطقة طلبات العملاء",
          ],
          adminFeatures: [
            "إدارة المنتجات",
            "إدارة الطلبات والشحن",
            "العروض وأكواد الخصم",
            "قوائم انتظار وتحليلات",
          ],
          pages: [
            "الرئيسية",
            "المتجر",
            "تفاصيل المنتج",
            "السلة",
            "الدفع",
            "طلباتي",
            "طلبات الإدارة",
            "منتجات الإدارة",
          ],
          techHighlights: [
            "واجهة متجر MERN",
            "مسارات محمية للعملاء والإدارة",
            "حسابات الخصومات والطلبات",
            "شبكة منتجات متجاوبة",
          ],
          tags: ["E-commerce", "Caps", "Admin"],
          showcaseTitles: [
            "واجهة متجر أنيقة",
            "استكشاف المنتجات",
            "مسار الدفع",
            "إدارة العمليات",
          ],
        },
        "s8-factory": {
          type: "موقع تعريفي للأعمال",
          businessType: "مصنع ملابس",
          status: "مميز",
          description:
            "موقع تعريفي جريء لمصنع ملابس مع مسار مباشر لبدء التعاون.",
          overview:
            "حضور جاد لمصنع ملابس مع مسارات إنتاج واضحة وأدوات إدارة عملية.",
          publicFeatures: [
            "صفحة رئيسية بحضور قوي",
            "عرض للعلامات والشركاء",
            "عرض خطوط الخامات",
            "مسارات طلب إنتاج وحجز مكالمة",
          ],
          adminFeatures: [
            "إدارة طلبات الإنتاج",
            "إعدادات مواعيد المكالمات والفترات",
            "العقود والعروض",
            "الآراء والتحليلات",
          ],
          pages: [
            "الرئيسية",
            "العلامات",
            "الخطوط",
            "المواعيد",
            "عن الشركة",
            "تواصل",
            "الحساب",
            "Admin",
          ],
          techHighlights: [
            "تدفق طلبات MERN",
            "مسارات محمية للحساب والإدارة",
            "إعدادات المواعيد",
            "تتبع العقود",
          ],
          tags: ["Business Website", "Factory", "Portal"],
          showcaseTitles: [
            "صفحة أعمال رئيسية",
            "خطوط الإنتاج",
            "تدفق الطلب والمكالمة",
            "مساحة الإدارة",
          ],
        },
        atheer: {
          type: "متجر إلكتروني",
          businessType: "متجر عطور",
          status: "مميز",
          description:
            "متجر عطور بطابع فاخر داكن مع منتجات ودفع وطلبات وأدوات إدارة.",
          overview:
            "واجهة متجر للعطور باتجاه بصري قوي وخلفية تجارة عملية.",
          publicFeatures: [
            "صفحة رئيسية فاخرة داكنة",
            "المتجر وتفاصيل المنتج",
            "السلة والدفع",
            "تتبع طلبات العملاء",
          ],
          adminFeatures: [
            "لوحة طلبات",
            "منتجات ورفع صور المنتجات",
            "عروض وأكواد خصم",
            "تحليلات وإدارة العملاء",
          ],
          pages: [
            "الرئيسية",
            "المتجر",
            "تفاصيل المنتج",
            "السلة",
            "الدفع",
            "طلباتي",
            "تواصل",
            "Admin",
          ],
          techHighlights: [
            "بيانات منتجات العطور",
            "حقول إثبات الدفع",
            "تدفق أكواد الخصم",
            "جاهزية رفع عبر Cloudinary",
          ],
          tags: ["E-commerce", "Perfume", "Admin"],
          showcaseTitles: [
            "واجهة متجر فاخرة",
            "تصفح العطور",
            "تجربة الطلب",
            "إدارة المتجر",
          ],
        },
        akm: {
          type: "متجر إلكتروني",
          businessType: "متجر ملابس",
          status: "مميز",
          description:
            "متجر أزياء نظيف مع خيارات منتجات وعروض ودفع وتتبع.",
          overview:
            "بناء تجاري يركز على خيارات المنتجات وطلبات العملاء وعمليات إدارة جادة.",
          publicFeatures: [
            "الصفحة الرئيسية مع دعم العربية",
            "المتجر وتفاصيل المنتج",
            "خيارات الألوان والمقاسات",
            "السلة والدفع",
          ],
          adminFeatures: [
            "إدارة المنتجات",
            "حالات الطلبات والدفع",
            "عروض وباقات وكوبونات وآراء",
            "حملات بريدية وإعدادات الموقع",
          ],
          pages: [
            "الرئيسية",
            "المتجر",
            "تفاصيل المنتج",
            "السلة",
            "الدفع",
            "طلباتي",
            "منتجات الإدارة",
            "طلبات الإدارة",
          ],
          techHighlights: [
            "تدفقات بيانات React Query",
            "نموذج مخزون للألوان والمقاسات",
            "تكاملات تتبع Meta/TikTok/Snap",
            "خدمة رفع Cloudinary",
          ],
          tags: ["E-commerce", "Fashion", "Tracking"],
          showcaseTitles: [
            "واجهة متجر أزياء",
            "خيارات المنتجات",
            "تجربة التسوق",
            "لوحة الإدارة",
          ],
        },
      },
    },
    process: {
      hero: {
        eyebrow: "الخطوات",
        title: "مسار واضح من الطلب إلى الإطلاق.",
        description:
          "خطوات بسيطة، عملية واضحة، واتجاه موقع يفهمه الجميع.",
      },
      questionsHeader: {
        eyebrow: "الأسئلة",
        title: "قبل أن نبدأ.",
        description: "إجابات سريعة عن طريقة تحرك المشروع.",
      },
      cta: {
        eyebrow: "ابدأ بوضوح",
        title: "أخبرنا بالهدف.",
        description:
          "وسنساعدك في تشكيل اتجاه الموقع المناسب من هناك.",
      },
      steps: [
        {
          number: "01",
          title: "الطلب",
          subtitle: "نبدأ بالأساسيات.",
          description:
            "أرسل هدف العمل ونوع الموقع وأي تفاصيل تعرفها بالفعل.",
          points: ["نوع الموقع", "هدف العمل", "الجدول الزمني"],
        },
        {
          number: "02",
          title: "الاتجاه",
          subtitle: "نشكّل الخطة.",
          description:
            "نحدد الصفحات والمميزات واحتياجات المحتوى والاتجاه البصري.",
          points: ["هيكل الصفحات", "الدعوة الأساسية", "نطاق المميزات"],
        },
        {
          number: "03",
          title: "البناء",
          subtitle: "تصميم وتطوير.",
          description:
            "يتم بناء الموقع بواجهة نظيفة وتخطيطات متجاوبة وتدفقات تعمل فعليًا.",
          points: ["واجهة متجاوبة", "نماذج أو تدفقات", "أدوات إدارة عند الحاجة"],
        },
        {
          number: "04",
          title: "الإطلاق",
          subtitle: "اختبار ثم نشر.",
          description:
            "نراجع الموبايل والنماذج والروابط وتفاصيل الإطلاق قبل نشر الموقع.",
          points: ["اختبار الموبايل", "فحص النماذج", "دعم الإطلاق"],
        },
      ],
      questions: [
        {
          question: "ماذا يحدث بعد إرسال الطلب؟",
          answer:
            "نراجع الهدف ونوع الموقع والتفاصيل التي أرسلتها، ثم نقترح الاتجاه والخطوة التالية.",
        },
        {
          question: "هل يجب أن يكون كل شيء جاهزًا قبل البدء؟",
          answer:
            "لا. إذا كانت الفكرة الأساسية موجودة، يمكننا مساعدتك في تشكيل الصفحات والهيكل واتجاه المحتوى.",
        },
        {
          question: "هل يمكنني حجز مكالمة أولًا؟",
          answer:
            "نعم. إذا أردت مناقشة المشروع قبل إرسال التفاصيل، يمكنك البدء بمكالمة.",
        },
        {
          question: "كيف تتم المراجعات؟",
          answer:
            "نشارك العمل على مراحل واضحة، نجمع ملاحظاتك، ثم نعدّل الموقع قبل الإطلاق.",
        },
        {
          question: "ماذا يحدث بعد الإطلاق؟",
          answer:
            "نتأكد أن الموقع منشور ويعمل بشكل صحيح وجاهز لاستقبال الزوار.",
        },
      ],
    },
    start: {
      hero: {
        eyebrow: "ابدأ",
        title: "ابدأ مشروعك.",
        description:
          "أرسل التفاصيل الآن، أو احجز مكالمة إذا أردت تشكيل الاتجاه أولًا.",
      },
      loggedIn: {
        title: "أنت تبدأ باسم {name}",
        description: "ستظهر طلباتك ومواعيدك داخل لوحة حسابك.",
        button: "عرض طلباتي",
      },
      options: {
        eyebrow: "اختر الطريقة",
        title: "ابدأ بالمسار الأنسب لك.",
        description:
          "أرسل الطلب الآن، أو احجز مكالمة إذا كان الاتجاه يحتاج إلى تشكيل أولًا.",
        footer:
          "يمكن للعملاء المسجلين متابعة الطلبات والمواعيد من لوحة حسابهم.",
        items: [
          {
            id: "request",
            title: "إرسال طلب مشروع",
            description: "الأفضل إذا كنت تعرف بالفعل نوع الموقع الذي تحتاجه.",
          },
          {
            id: "call",
            title: "حجز مكالمة",
            description: "الأفضل إذا أردت مناقشة الاتجاه أولًا.",
          },
        ],
      },
      requestForm: {
        eyebrow: "طلب موقع",
        title: "أخبرنا بما تحتاجه.",
        description:
          "شاركنا النشاط والهدف واتجاه الموقع. سنراجع التفاصيل ونتواصل معك بالخطوة التالية.",
        name: "الاسم *",
        namePlaceholder: "اسمك",
        businessName: "اسم النشاط",
        businessNamePlaceholder: "اسم النشاط / العلامة التجارية",
        phone: "الهاتف / WhatsApp *",
        phonePlaceholder: "011...",
        email: "البريد الإلكتروني *",
        emailPlaceholder: "you@example.com",
        websiteType: "نوع الموقع *",
        preferredContact: "طريقة التواصل المفضلة",
        identity: "هل لديك شعار / هوية بصرية؟",
        content: "هل المحتوى / المنتجات / الصور جاهزة؟",
        budget: "نطاق الميزانية",
        optional: "اختياري",
        deadline: "الموعد النهائي",
        details: "تفاصيل المشروع *",
        detailsPlaceholder:
          "أخبرنا عن نشاطك، هدف الموقع، الصفحات الأساسية، وأهم المميزات.",
        submit: "إرسال طلب الموقع",
        submitting: "جار الإرسال...",
        validation:
          "يرجى إدخال الاسم والهاتف والبريد الإلكتروني وتفاصيل المشروع.",
        successLoggedIn:
          "تم إرسال طلب الموقع. يمكنك متابعته من لوحة حسابك.",
        success: "تم إرسال طلب الموقع بنجاح.",
        error: "تعذر إرسال طلب الموقع.",
      },
      callForm: {
        eyebrow: "حجز مكالمة",
        title: "اختر وقتًا للحديث.",
        description: "اختر موعدًا متاحًا وأخبرنا بما تريد مناقشته.",
        bookingAs:
          "أنت تحجز باسم {name}. سيظهر هذا الموعد داخل لوحة حسابك.",
        availableSlots: "المواعيد المتاحة",
        topic: "ما الذي تريد مناقشته؟ *",
        topicPlaceholder:
          "مثال: أريد متجرًا إلكترونيًا لعلامتي التجارية",
        notes: "ملاحظات اختيارية",
        notesPlaceholder: "أي تفاصيل إضافية قبل المكالمة؟",
        submit: "حجز موعد مكالمة",
        submitting: "جار الحجز...",
        myAppointments: "مواعيدي",
        validationSlot: "يرجى اختيار موعد للمكالمة.",
        validationDetails:
          "يرجى إدخال الاسم والهاتف والبريد الإلكتروني وموضوع المناقشة.",
        loadError: "تعذر تحميل مواعيد المكالمات المتاحة.",
        successLoggedIn:
          "تم حجز موعد المكالمة. يمكنك متابعته من لوحة حسابك.",
        success: "تم حجز موعد المكالمة بنجاح.",
        error: "تعذر حجز الموعد.",
      },
      slots: {
        loading: "جار تحميل مواعيد المكالمات المتاحة...",
        emptyTitle: "لا توجد مواعيد مكالمات متاحة حاليًا.",
        emptyDescription:
          "يمكنك إرسال طلب موقع أو التواصل مع Web District مباشرة عبر WhatsApp.",
        chooseDay: "اختر اليوم",
        nextDays: "أقرب 5 أيام متاحة",
        available: "متاح",
      },
    },
    contact: {
      hero: {
        eyebrow: "تواصل معنا",
        title: "أخبرنا بما تحتاجه.",
        description:
          "راسلنا مباشرة، أو استخدم صفحة ابدأ إذا كنت تعرف اتجاه الموقع بالفعل.",
      },
      whatsappMessage:
        "مرحبًا Web District، أريد الاستفسار عن بناء موقع.",
      cards: [
        {
          title: "WhatsApp",
          description: "راسلنا مباشرة.",
        },
        {
          title: "الهاتف",
          description: "اتصل لمحادثة سريعة.",
        },
        {
          title: "البريد الإلكتروني",
          description: "أرسل تفاصيل المشروع.",
        },
        {
          title: "Instagram",
          description: "تابعنا أو أرسل رسالة مباشرة.",
        },
      ],
    },
    auth: {
      login: {
        eyebrow: "تسجيل الدخول",
        title: "ادخل إلى حسابك في Web District.",
        description:
          "سجّل الدخول لمتابعة الطلبات والمواعيد والعقود وتحديثات المشروع.",
        email: "البريد الإلكتروني",
        password: "كلمة المرور",
        passwordPlaceholder: "أدخل كلمة المرور",
        forgotPassword: "نسيت كلمة المرور؟",
        submit: "تسجيل الدخول",
        submitting: "جار تسجيل الدخول...",
        noAccount: "ليس لديك حساب؟",
        createOne: "أنشئ حسابًا",
        validation: "يرجى إدخال البريد الإلكتروني وكلمة المرور.",
        error: "تعذر تسجيل الدخول.",
      },
      signup: {
        eyebrow: "إنشاء حساب",
        title: "أنشئ حساب العميل الخاص بك.",
        description:
          "استخدم حسابك لاحقًا لمتابعة الطلبات والمواعيد والعقود وتحديثات المشروع.",
        password: "كلمة المرور *",
        passwordPlaceholder: "6 أحرف على الأقل",
        submit: "إنشاء الحساب",
        submitting: "جار إنشاء الحساب...",
        haveAccount: "لديك حساب بالفعل؟",
        login: "تسجيل الدخول",
        validation: "يرجى إدخال الاسم والبريد الإلكتروني وكلمة المرور.",
        passwordLength: "يجب أن تكون كلمة المرور 6 أحرف على الأقل.",
        error: "تعذر إنشاء الحساب.",
      },
      forgot: {
        eyebrow: "استعادة كلمة المرور",
        title: "إعادة تعيين كلمة المرور.",
        description:
          "أدخل بريد حسابك وسنرسل رابطًا آمنًا لإعادة التعيين إذا كان الحساب موجودًا.",
        submit: "إرسال رابط التعيين",
        submitting: "جار الإرسال...",
        remember: "تذكرت كلمة المرور؟",
        success: "تم إرسال تعليمات إعادة التعيين.",
        validation: "يرجى إدخال بريدك الإلكتروني.",
        error: "تعذر إرسال تعليمات إعادة التعيين.",
      },
      reset: {
        eyebrow: "كلمة مرور جديدة",
        title: "أنشئ كلمة مرور جديدة.",
        description:
          "اختر كلمة مرور آمنة لاستعادة الدخول إلى حسابك في Web District.",
        newPassword: "كلمة المرور الجديدة",
        confirmPassword: "تأكيد كلمة المرور",
        confirmPlaceholder: "أعد كتابة كلمة المرور الجديدة",
        submit: "إعادة تعيين كلمة المرور",
        submitting: "جار إعادة التعيين...",
        goBack: "العودة إلى",
        validation: "يرجى إدخال كلمة المرور الجديدة وتأكيدها.",
        noMatch: "كلمتا المرور غير متطابقتين.",
        passwordLength: "يجب أن تكون كلمة المرور 6 أحرف على الأقل.",
        success: "تم إعادة تعيين كلمة المرور بنجاح.",
        error:
          "تعذر إعادة تعيين كلمة المرور. قد يكون الرابط غير صالح أو منتهي الصلاحية.",
      },
    },
    client: {
      layout: {
        portal: "بوابة العميل",
        welcome: "مرحبًا، {name}",
        goHome: "العودة للرئيسية",
        logout: "تسجيل الخروج",
        links: {
          overview: "نظرة عامة",
          requests: "الطلبات",
          appointments: "المواعيد",
          contracts: "العقود",
          projectStatus: "حالة المشروع",
          reviews: "الآراء",
          profile: "الملف الشخصي",
        },
      },
      dashboard: {
        eyebrow: "نظرة عامة",
        title: "مساحة عملك مع Web District.",
        description:
          "تابع طلبات الموقع ومواعيد المكالمات والآراء والعقود القادمة من بوابة عميل واحدة وواضحة.",
        submitNew: "إرسال طلب جديد",
        viewRequests: "عرض الطلبات",
        businessFallback: "غير مضاف",
        actions: [
          {
            title: "طلبات المواقع",
            description: "شاهد الطلبات التي أرسلتها أثناء تسجيل الدخول.",
          },
          {
            title: "مواعيد المكالمات",
            description: "تابع المواعيد المحجوزة وحالة كل موعد.",
          },
          {
            title: "الآراء",
            description: "أرسل رأيك عن Web District بعد العمل معًا.",
          },
          {
            title: "الملف الشخصي",
            description: "اعرض بيانات حسابك ونشاطك.",
          },
        ],
        nextEyebrow: "ماذا بعد؟",
        nextTitle: "ابدأ بطلب أو مكالمة.",
        nextDescription:
          "إذا كنت تعرف الموقع الذي تحتاجه، أرسل طلب موقع. وإذا لم تكن متأكدًا بعد، احجز مكالمة وسيساعدك فريق Web District في تشكيل الاتجاه المناسب قبل الوصول إلى عرض أو عقد.",
        steps: [
          "إرسال طلب",
          "Web District تراجع التفاصيل",
          "نوضح النطاق والخطوة التالية",
          "يتم تجهيز عرض أو عقد",
        ],
      },
      requests: {
        title: "طلبات المواقع الخاصة بك",
        description:
          "تابع الطلبات التي أرسلتها إلى Web District وحالتها الحالية.",
        loading: "جار تحميل طلبات المواقع...",
        loadError: "تعذر تحميل طلباتك.",
        emptyTitle: "لا توجد طلبات مواقع بعد",
        emptyDescription:
          "عند إرسال طلب موقع، سيظهر هنا مع حالته الحالية.",
      },
      appointments: {
        title: "مواعيد المكالمات الخاصة بك",
        description:
          "تابع مكالماتك المحجوزة مع Web District وحالتها الحالية.",
        bookAnother: "احجز مكالمة أخرى",
        loading: "جار تحميل مواعيد المكالمات...",
        loadError: "تعذر تحميل مواعيدك.",
        emptyTitle: "لا توجد مواعيد مكالمات بعد",
        emptyDescription:
          "عند حجز موعد مكالمة، سيظهر هنا مع حالته وتفاصيله.",
      },
      contracts: {
        title: "العقود والعروض",
        description:
          "اعرض العروض ونطاق المشروع وتفاصيل الدفع والجدول الزمني وحالة العقد التي أعدتها Web District. يمكنك قبول العرض أو إرسال ملاحظة من هنا.",
        loading: "جار تحميل العقود...",
        loadError: "تعذر تحميل عقودك.",
        emptyTitle: "لا توجد عقود بعد",
        emptyClient: "ستظهر هنا العقود والعروض التي تجهزها Web District.",
        emptyAdmin:
          "ستظهر هنا العقود التي يتم إنشاؤها من الطلبات أو المواعيد.",
        acceptTitle: "قبول العرض؟",
        acceptMessage:
          "سيتم وضع عرض Web District كعرض مقبول وإبلاغ الفريق.",
        acceptConfirm: "قبول",
        acceptedSuccess: "تم قبول العقد بنجاح.",
        acceptError: "تعذر قبول العقد.",
        noteSaved: "تم حفظ ملاحظتك بنجاح.",
        noteError: "تعذر حفظ الملاحظة.",
        acceptProposal: "قبول العرض",
        accepting: "جار القبول...",
        sendNote: "إرسال / تعديل ملاحظة",
        noteLabel: "ملاحظتك إلى Web District",
        notePlaceholder:
          "اكتب أي سؤال أو تعليق أو توضيح بخصوص هذا العرض...",
        saveNote: "حفظ الملاحظة",
        saving: "جار الحفظ...",
      },
      projectStatus: {
        title: "حالة المشروع",
        description:
          "تابع تقدم العروض والمشاريع النشطة من خلال حالة العقد.",
        viewContracts: "عرض العقود",
        loading: "جار تحميل حالة المشروع...",
        loadError: "تعذر تحميل حالة المشروع.",
        emptyTitle: "لا توجد حالة مشروع نشطة بعد",
        emptyDescription:
          "عندما تجهز Web District عرضًا أو عقدًا لك، ستظهر حالة المشروع هنا.",
      },
      reviews: {
        title: "إرسال رأي",
        description:
          "شارك تجربتك مع Web District. سيظهر رأيك علنًا بعد اعتماد الإدارة فقط.",
        note:
          "الآراء متاحة لحسابات العملاء التي لديها عقد واحد على الأقل. تتم مراجعة كل رأي قبل ظهوره علنًا.",
        checking: "جار التحقق من إمكانية إرسال رأي...",
        loadError: "تعذر التحقق من صلاحية إرسال الرأي.",
        lockedTitle: "إرسال الرأي غير متاح حاليًا.",
        lockedDescription:
          "يحتاج حسابك إلى عقد واحد على الأقل مع Web District قبل إرسال رأي.",
        businessPlaceholder: "نشاطك / علامتك التجارية",
        role: "الدور",
        rolePlaceholder: "Founder / Owner / Manager",
        rating: "التقييم",
        stars: "{count} نجوم",
        message: "نص الرأي *",
        messagePlaceholder:
          "مثال: ساعدتنا Web District في إطلاق موقع أنظف وأكثر احترافية.",
        submit: "إرسال الرأي",
        submitting: "جار الإرسال...",
        validation: "يرجى كتابة نص الرأي.",
        success: "تم إرسال الرأي وهو في انتظار الاعتماد.",
        error: "تعذر إرسال الرأي.",
      },
      profile: {
        title: "بيانات الملف الشخصي",
        description:
          "هذه هي بيانات الحساب المرتبطة بمساحة عملك مع Web District. الطلبات والمكالمات والآراء والعقود القادمة مرتبطة بهذا الحساب.",
        startNew: "ابدأ طلبًا جديدًا",
        clientFallback: "عميل",
        noBusiness: "لم تتم إضافة اسم نشاط",
        noteEyebrow: "ملاحظة الحساب",
        noteTitle: "حافظ على ربط طلباتك.",
        noteDescription:
          "عند إرسال طلب موقع أو حجز مكالمة أثناء تسجيل الدخول، يظهر ذلك تلقائيًا في لوحة العميل. إذا أرسلت وأنت غير مسجل، سيصل الطلب إلى Web District، لكنه لن يظهر داخل هذا الحساب.",
        quickCards: [
          {
            title: "الطلبات",
            description: "عرض طلبات المواقع المرتبطة بحسابك.",
          },
          {
            title: "المكالمات",
            description: "متابعة المواعيد المحجوزة وحالة المكالمات.",
          },
          {
            title: "الآراء",
            description: "إرسال رأيك بعد العمل مع Web District.",
          },
        ],
      },
    },
    success: {
      metaTitle: "تم بنجاح",
      metaDescription: "تم إرسال طلبك إلى Web District بنجاح.",
      next: "ماذا يحدث بعد ذلك",
      startAnother: "ابدأ طلبًا آخر",
      call: {
        eyebrow: "تم حجز المكالمة",
        title: "تم حجز موعد المكالمة.",
        description:
          "استلمنا تفاصيل المكالمة. سنراجع الاتجاه ونتواصل معك إذا احتاج أي شيء إلى تأكيد.",
        steps: [
          "نراجع موضوع المكالمة.",
          "نحضّر الأسئلة المناسبة قبل الموعد.",
          "نستخدم المكالمة لتشكيل اتجاه الموقع.",
        ],
        accountLabel: "عرض الموعد",
      },
      request: {
        eyebrow: "تم إرسال الطلب",
        title: "تم إرسال طلب الموقع.",
        description:
          "استلمنا تفاصيل مشروعك. سنراجع هدف العمل ونوع الموقع والاتجاه قبل الرد.",
        steps: [
          "نراجع طلب الموقع.",
          "نشكّل أفضل اتجاه للمشروع.",
          "نتواصل معك بالخطوة التالية.",
        ],
        accountLabel: "عرض الطلب",
      },
    },
    legal: {
      terms: {
        metaTitle: "الشروط والأحكام",
        metaDescription:
          "اقرأ شروط Web District الواضحة للطلبات والعروض والدفع والجداول والمراجعات والإطلاق والدعم الشهري.",
        eyebrow: "الشروط",
        title: "الشروط والأحكام",
        description:
          "شروط خدمة بسيطة وواضحة للعمل مع Web District من أول طلب وحتى الإطلاق والدعم.",
        ctaEyebrow: "تواصل",
        ctaTitle: "هل لديك سؤال عن هذه الشروط؟",
        ctaDescription:
          "ابدأ مشروعك عندما تكون جاهزًا، أو تواصل مع Web District لأي سؤال، أو راجع خدمة العناية بالموقع داخل صفحة الخدمات للدعم بعد الإطلاق.",
        sections: [
          {
            title: "الخدمات",
            body: [
              "تقدم Web District خدمات تصميم وتطوير المواقع والمتاجر الإلكترونية وصفحات الهبوط والحلول المخصصة حسب نطاق كل مشروع.",
              "يتم الاتفاق على تفاصيل الخدمة قبل بدء العمل.",
            ],
          },
          {
            title: "طلبات المشاريع والعروض",
            body: [
              "إرسال طلب أو نموذج أو رسالة لا يعني قبول المشروع تلقائيًا.",
              "بعد مراجعة التفاصيل، قد تجهز Web District عرضًا أو تسعيرًا أو عقدًا بناءً على نطاق المشروع.",
            ],
          },
          {
            title: "المدفوعات والدفعات المقدمة",
            body: [
              "يتم الاتفاق على شروط الدفع والدفعات والمراحل ومواعيد التسليم لكل مشروع.",
              "قد يعتمد بدء العمل أو مرحلة معينة على استلام الدفعة المتفق عليها.",
            ],
          },
          {
            title: "الجداول والتأخيرات",
            body: [
              "تُبنى جداول المشروع حول النطاق المتفق عليه والمحتوى والملاحظات والاحتياجات التقنية.",
              "تأخر ملاحظات العميل أو المحتوى أو الصور أو الموافقات أو صلاحيات الجهات الخارجية قد يؤثر على مواعيد التسليم.",
            ],
          },
          {
            title: "مسؤوليات العميل",
            body: [
              "يتحمل العميل مسؤولية تقديم معلومات النشاط والمحتوى والصور وملفات الهوية والصلاحيات والموافقات بشكل صحيح.",
              "إذا ساعدت Web District في تشكيل اتجاه المحتوى، يبقى العميل مسؤولًا عن تأكيد دقته قبل الإطلاق.",
            ],
          },
          {
            title: "المراجعات والتعديلات البسيطة",
            body: [
              "تتم المراجعات المعقولة حسب اتفاق المشروع ومرحلة العمل.",
              "التعديلات البسيطة تختلف عن المميزات الكبيرة الجديدة أو إعادة بناء الصفحات أو تغيير الاتجاه ما لم يتم الاتفاق على ذلك منفصلًا.",
            ],
          },
          {
            title: "محتوى الموقع والملفات",
            body: [
              "يجب أن يقدم العميل محتوى أو صورًا أو شعارات أو خطوطًا أو ملفات يملكها أو لديه حق استخدامها.",
              "قد تعرض Web District الأعمال المكتملة في معرض أعمالها ما لم يطلب العميل غير ذلك.",
            ],
          },
          {
            title: "خدمات الطرف الثالث",
            body: [
              "قد تستخدم المشاريع خدمات خارجية مثل الاستضافة والدومين والبريد والتحليلات ومنصات الدفع والإضافات وواجهات API.",
              "قد يكون لهذه الخدمات أسعار وشروط وحدود وتوفر خاص بها.",
            ],
          },
          {
            title: "إطلاق الموقع",
            body: [
              "قبل الإطلاق، تراجع Web District الصفحات المتفق عليها والتجاوب والروابط الأساسية والنماذج أو التدفقات الرئيسية.",
              "يعتمد الإطلاق النهائي على الصلاحيات والموافقات وجاهزية الدومين أو الاستضافة وأي خدمات خارجية مرتبطة.",
            ],
          },
          {
            title: "الدعم الشهري / العناية بالموقع",
            body: [
              "الدعم الشهري أو العناية بالموقع خدمة منفصلة ما لم تكن مشمولة في اتفاق المشروع.",
              "قد تشمل خطط العناية تعديلات بسيطة وتحديثات وفحوصات وتوجيه. المميزات الكبيرة الجديدة لا تُعد تعديلات شهرية بسيطة إلا إذا تم الاتفاق عليها.",
            ],
          },
        ],
      },
      privacy: {
        metaTitle: "سياسة الخصوصية",
        metaDescription:
          "تعرّف على كيفية جمع Web District للمعلومات واستخدامها من خلال النماذج وحسابات العملاء والرسائل وطلبات المواقع.",
        eyebrow: "الخصوصية",
        title: "سياسة الخصوصية",
        description:
          "نظرة واضحة على المعلومات التي نجمعها وكيف تساعدنا في الرد والتخطيط ودعم مشروع موقعك.",
        ctaEyebrow: "تواصل",
        ctaTitle: "تريد السؤال عن بياناتك؟",
        ctaDescription:
          "ابدأ مشروعك عندما تكون جاهزًا، أو تواصل مع Web District إذا كان لديك سؤال عن الخصوصية قبل المتابعة.",
        sections: [
          {
            title: "المعلومات التي نجمعها",
            body: [
              "قد نجمع التفاصيل التي ترسلها مثل الاسم والبريد والهاتف ورقم WhatsApp واسم النشاط واحتياجات الموقع وتفاصيل المواعيد ومعلومات المشروع.",
              "إذا أنشأت حساب عميل، قد نخزن أيضًا بيانات الحساب اللازمة لإدارة لوحة حسابك وطلباتك.",
            ],
          },
          {
            title: "كيف نستخدم المعلومات",
            body: [
              "نستخدم المعلومات للرد على الرسائل ومراجعة طلبات المشاريع وإدارة المواعيد وتجهيز العروض أو العقود وتقديم الخدمات والتواصل بخصوص مشروعك.",
              "قد نستخدم نشاط الموقع بشكل عام لتحسين تجربة الموقع.",
            ],
          },
          {
            title: "النماذج وطلبات المواقع",
            body: [
              "عند إرسال طلب موقع أو نموذج تواصل، تساعدنا المعلومات في فهم أهدافك واقتراح الخطوة المناسبة.",
              "يرجى عدم إرسال معلومات شخصية حساسة إلا إذا كانت ضرورية لطلبك.",
            ],
          },
          {
            title: "حسابات العملاء",
            body: [
              "قد تعرض حسابات العملاء الطلبات والمواعيد والعقود وحالة المشروع والآراء وبيانات الملف المرتبطة بعملك مع Web District.",
              "يجب على المستخدم الحفاظ على خصوصية بيانات الدخول.",
            ],
          },
          {
            title: "ملاحظة عن الكوكيز والتحليلات",
            body: [
              "قد يستخدم الموقع كوكيز بسيطة أو تحليلات أو أدوات مشابهة لفهم الأداء ونشاط الزوار.",
              "تساعدنا هذه الأدوات في تحسين الموقع ولا تغير أسعار الخدمات أو توفرها.",
            ],
          },
          {
            title: "البريد والتواصل",
            body: [
              "إذا تواصلت معنا عبر البريد أو WhatsApp أو Instagram أو الهاتف أو نموذج الموقع، قد نحتفظ بالمحادثة حتى نتمكن من الرد وإدارة المشروع بشكل صحيح.",
              "يمكنك طلب تحديث أو حذف بيانات التواصل غير الضرورية.",
            ],
          },
          {
            title: "تخزين البيانات",
            body: [
              "قد تُخزن المعلومات في نظام الموقع أو البريد أو سجلات المشروع أو بيئة الاستضافة أو أدوات العمل المعتمدة.",
              "نسعى للاحتفاظ بالمعلومات فقط للمدة اللازمة للتواصل أو تقديم الخدمة أو السجلات أو أغراض العمل المعقولة.",
            ],
          },
          {
            title: "مشاركة المعلومات",
            body: [
              "لا نبيع المعلومات الشخصية.",
              "قد نستخدم خدمات خارجية مثل الاستضافة والبريد والتحليلات ومنصات الدفع أو التواصل والأدوات التقنية عند الحاجة لتشغيل الموقع وتقديم الخدمات.",
            ],
          },
          {
            title: "حقوق المستخدم",
            body: [
              "يمكنك التواصل معنا للسؤال عن المعلومات التي نحتفظ بها أو طلب تصحيحها أو حذف ما يمكن حذفه بشكل معقول.",
              "قد نحتاج للاحتفاظ ببعض السجلات لأسباب متعلقة بالمشروع أو العقد أو الأمان أو العمل.",
            ],
          },
        ],
      },
    },
    notFound: {
      metaTitle: "الصفحة غير موجودة",
      metaDescription: "الصفحة التي تبحث عنها غير موجودة على Web District.",
      title: "هذه الصفحة خرجت عن المسار.",
      description:
        "الصفحة التي تبحث عنها غير موجودة، أو تم نقلها، أو أن الرابط غير صحيح.",
    },
  },
};
