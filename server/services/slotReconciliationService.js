...
const { bookingWindowEnd } = require("../utils/slotTime");
...
const reconcileSlots = async () => {
  ...
  const [holdingAppointments, bookedSlots, duplicateHolders, expiredCandidates, outsideHorizon, malformed, activeSlots] = await Promise.all([
    ...
    CallSlot.find({ endsAt: { $lt: now }, isBooked: false, bookedBy: null, protectedFromCleanup: { $ne: true } }).select("_id").limit(1000).lean(),
    CallSlot.find({ startsAt: { $gt: bookingWindowEnd(now) }, isAutoGenerated: true, isBooked: false }).select("_id").limit(1000).lean(),
